...
function TableAdmin({
  ...
  const pageSizeOptions = [15, 25, 50, 75, 100, 125].map(
    (value) => `Show ${value}`,
  );
    ...
    <section className="sm:flex mb-10 justify-between">
      <div className="sm:flex sm:space-x-4">
        ...
        <Popup label={`Filter`} classStyle={"mt-5 sm:mt-0"}>
          <ListItems
            mapData={generationOptions}
            handleList={handlePokeGeneration}
          />
        </Popup>
      </div>
      ...
      <div className="mt-5 sm:mt-0 sm:flex relative">
        ...
        />
        <input
          className={`shadow appearance-none border rounded w-full py-2 pl-10 leading-tight focus:outline-none focus:shadow-outline
            ${
              dark
