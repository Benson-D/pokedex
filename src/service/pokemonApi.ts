...
class PokemonAPI {
     ...
     */
    public static async getPokemonStat(pokemonList: PokemonListOutput[]): Promise<FormattedPokemon[]> {
        return await Promise.all(
            pokemonList.map( async pokemon => {
                const pokeStats: any = await this.getPokemonStats(pokemon.url);
                const types: string[] = pokeStats.types.map((stat: any) => stat.type.name);
    ...
    public static formatPokemon(pokemonData: Pokemon): FormattedPokemon {
        ...
        const sprites = JSON.parse(pokemonData.pokemon_v2_pokemonsprites[0].sprites) as PokemonSprites;
        ...
        const attacks = pokemonData.pokemon_v2_pokemonmoves_aggregate.aggregate.count ?? 0;
